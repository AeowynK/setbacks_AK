...
class gwModels:
    ...
    def f_mils(self, t):
        ...
        ulim = 5  # can't use np.inf as upper limit
        integrand = []
...
def glhe_groundwater_model(times, params, config, load):
    ...
    if (config.nx_obs > 1) or (config.ny_obs > 1):
        ...
        print("these are the x-axis observation locations (with the property line being 0): ", x_obs)
        print("these are the y-axis observation locations (same as above): ", y_obs)
    else:
        obs_grid = [[config.x_obs, config.y_obs]]

    s = []
    # creating an empty array for later storage of desired output 
